...
7. **중요 — 블로거 개인 사정을 가게의 장단점으로 분류하지 마세요:**
   - "거리가 멀다", "집에서 멀다", "자주 방문하기 어렵다" → 블로거의 위치 문제이지 가게 단점이 아님 → 제외
   - "주차가 어렵다", "대중교통 접근성이 낮다" → 가게의 입지 특성이므로 단점으로 포함 가능
   - "개인 취향에 안 맞는 음악", "나한테는 가격이 비쌌다" → 개인 호불호로 분류
   - 핵심 기준: "이 가게가 바꿀 수 있는 것인가?" — 가게가 바꿀 수 없는 블로거 개인의 상황은 단점이 아님
